...
import { INIT_OPACITY, defaultIframePageState, defaultMaskState } from "~common/defaultState";
...
function IndexPopup() {
  const [state, setState] = useStorage<MaskState>(MASK_STORAGE, defaultMaskState);
  const progressRef = React.useRef<HTMLProgressElement>(null);
  const [url, setUrl] = useState("");
  const [iframeState, setIframeState] = useStorage<IframePageState>(IFRAME_PAGE_STORAGE, defaultIframePageState);
  ...
  return (
    <div className="py-3 bg-black w-60 text-gray-400">
      <div
        className={`${!isUrlActive ? " line-through" : ""} mx-2 p-2 mb-4 rounded-sm border-b border-indigo-500 border-solid hover:bg-gray-800 cursor-pointer`}
        onClick={() => {
          if (isUrlActive) {
      ...
      <div className="mt-4 px-4 space-y-2">
        ...
        <div className="flex items-center gap-x-2">
          <input
            type="text"
            className="flex-1 h-7 px-2 py-1 bg-zinc-600 rounded-sm border-none outline-none"
            value={iframeState.url}
            onChange={(e) => setIframeState({ ...iframeState, url: e.target.value })}
          />
          <Button onClick={() => setIframeState({ ...iframeState, isOpen: !iframeState.isOpen })}>
            {iframeState.isOpen ? "隐藏" : "注入"}
          </Button>
        </div>
      </div>
      <div className="mt-4 px-4 text-xs space-y-2">
        <div>
